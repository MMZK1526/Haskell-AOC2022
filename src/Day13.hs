...
mkSignals str = let (signal1, "") = work str1; (signal2, "") = work str2
  ...
  where
    (str1, '\n' : str2) = break (== '\n') str
    work str
      | Just n <- readMaybe (takeWhile (`elem` ['0'..'9']) str)
        = (Val n, dropWhile (`elem` ['0'..'9']) str)
      | otherwise = listSignal str
    ...
    manySignals str     = case head str of
      ...
      ']' -> ([], str)
      _   -> let (signal, rest) = work str
                 (signals, rem) = manySignals rest
...
day13Part2 signals = (1 + S.findIndex (List [Val 2]) sigSet)
  ...
  where
    sigSet = S.union (S.fromList (concatMap (\(a, b) -> [a, b]) signals))
