...
simulateOne :: Int -> STArray s Int IntSet
            -> StateT SimulationState (ST s) Int
simulateOne count boardST = do
  (h, (ix, shape) ::: shapes, dirs, cache) <- get
  let cache' = M.insert (ix, fst $ I.head dirs) (count, h) cache
  (h', dirs')                              <- fall (shape (h + 4)) dirs
  put (max h h', shapes, dirs', cache')
  return $ count + 1
  where
    isIn (x, y)
      | x < -3 || x > 3 = pure False
...
simulation count dirs = runST $ do
  boardST <- A.newArray (-3, 3) $ IS.singleton 0
  ((ni, oi), (nh, oh))    <- takeArith <$> evalStateT (worker 1 boardST) init
  -- error $ show $ I.take 10 list
  -- let ((ni, oi), (nh, oh)) = takeArith list
  if count < oh
    then pure $ naiveSimulation count dirs
    else do
      let (repeatCount, remainder) = quotRem (count - oh) (ni - oi)
      return $ repeatCount * (nh - oh) + naiveSimulation (oh + remainder) dirs
  where
    ...
    worker i boardST = do
      ...
      case cache M.!? (sIx, dIx) of
        Nothing -> worker (i + 1) boardST
        Just x  -> do
          modify (\(h, s, d, _) -> (h, s, d,  M.singleton (sIx, dIx) (i, h)))
          (x :::) <$> worker (i + 1) boardST
